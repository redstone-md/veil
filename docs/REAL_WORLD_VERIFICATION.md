...
|---|---|
| Date | 2026-05-04 |
| Veil revision | `dev-lt-test` (commit `a76bf89`, `master`) |
| Veil binary build | `GOOS=linux GOARCH=amd64 CGO_ENABLED=0 -trimpath -ldflags "-s -w"` |
| Server | Debian 13 (trixie), kernel 6.12.57, KVM VPS, 967 MiB RAM, 15 GiB disk |
| Server location | Vilnius, Lithuania (ASN 44477) |
| Server IP | `vps.example.com` (single IPv4, no IPv6 enabled) |
| Server hosting | Hostinger (Ferrum LT) |
| Client | Local development machine, Czechia (188.122.209.9) |
...
| Path | Direct (CZ →) | Via Veil (CZ → LT →) | Overhead |
|------|---------------|----------------------|----------|
...
The Helsinki overhead is dominated by the extra Czechia → Lithuania
...
`openssl s_client -connect vps.example.com:443 -servername www.cloudflare.com`
...
`curl https://www.cloudflare.com/ --resolve …:vps.example.com` returned
the genuine Cloudflare HTML — including a `country: "LT"` field in
Cloudflare's geo-detection block, because the splice path made the
request from the VPS itself.
...
- iOS / Android / desktop GUI clients (none yet exist).
...
YAML
  veil admin user-create --db /var/lib/veil/users.db \
    --username root --password "<choose>"
  veil user add --db /var/lib/veil/users.db \
    --name testuser --pubkey "<client-static-pubkey-b64>"'
```
